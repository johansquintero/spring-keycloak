...
public class JwtAuthenticationConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    public AbstractAuthenticationToken convert(@NonNull  Jwt jwt) {
        Collection<GrantedAuthority> grantedAuthorities = Stream
                .concat(jwtGrantedAuthoritiesConverter.convert(jwt).stream(), extractResourceRoles(jwt).stream())
                .toList();
        return new JwtAuthenticationToken(jwt,grantedAuthorities,getPrincipalName(jwt));
    }
    ...
    private Collection<? extends GrantedAuthority> extractResourceRoles(@NonNull Jwt jwt) {
        ...
        return resourceRoles.stream().map(
